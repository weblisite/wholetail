-- Wholetail Database Schema for Neon PostgreSQL
-- This file contains the complete database schema for the Wholetail B2B Platform
-- Migrated from Supabase to Neon

-- Create custom types
CREATE TYPE user_type AS ENUM ('wholesaler', 'retailer', 'farmer', 'financier');
...
CREATE TABLE users (
    ...
    clerk_id VARCHAR(255) NOT NULL UNIQUE, -- Explicit Clerk ID reference
...
CREATE TABLE products (
    ...
    supplier_id VARCHAR(255) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    description TEXT,
...
CREATE TABLE orders (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    retailer_id VARCHAR(255) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    supplier_id VARCHAR(255) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE financing (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    retailer_id VARCHAR(255) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    financier_id VARCHAR(255) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE OR REPLACE FUNCTION calculate_confidence_score(retailer_user_id VARCHAR(255))
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
    ...
    FROM orders 
    WHERE retailer_id = retailer_user_id;
    ...
    VALUES (retailer_user_id, purchase_score, repayment_score, activity_score, total_score)
...
CREATE OR REPLACE FUNCTION update_supplier_rating(supplier_user_id VARCHAR(255))
RETURNS VOID AS $$
DECLARE
    avg_rating DECIMAL;
...
BEGIN
    ...
    WHERE p.supplier_id = supplier_user_id;
    
    INSERT INTO supplier_ratings (supplier_id, average_rating, total_reviews)
    VALUES (supplier_user_id, COALESCE(avg_rating, 0), COALESCE(review_count, 0))
...
-- This schema provides a comprehensive foundation for the Wholetail platform on Neon
-- Key changes from Supabase:
-- 1. User IDs changed from UUID to VARCHAR(255) to accommodate Clerk user IDs
-- 2. Added clerk_id field for explicit Clerk reference
-- 3. Removed Row Level Security (handled by application layer)
-- 4. Added farmer to user_type enum